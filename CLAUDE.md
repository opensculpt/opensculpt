...
**START HERE: Read `.opensculpt/DEMANDS.md`** — this file is auto-generated by the OS with current problems it needs fixed. It's updated every time the evolution engine escalates a demand. If it exists and has active demands, fix those first before doing anything else.

1. **Check demands**: Read `.opensculpt/DEMANDS.md` (auto-updated) or `curl localhost:8420/api/evolution/demands`
...
5. **Read `.opensculpt/skills/`** — evolved skills and patterns the OS has learned
6. **Write your fix**: new tool → `agos/tools/`, new daemon → `agos/daemons/`, bug fix → modify source directly
7. **Run tests**: `python -m pytest tests/ --ignore=tests/test_frontend_playwright.py -q`
8. **Verify**: `sculpt verify` — checks if demands were resolved
9. **Hot-load**: New tools in `.opensculpt/evolved/` get auto-loaded. Source changes need restart.
10. **Share**: `sculpt contribute` — shares your fix with the fleet
